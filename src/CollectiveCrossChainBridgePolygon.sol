...
contract CollectiveCrossChainBridgePolygon is Withdraw {
    ...
    function sendBalances() public {
        ...
        for (uint256 i = 0; i < rounds[currentRound].participants.length; i++) {
            fxTokenAddress.safeTrasfer(participants[i], rounds[currentRound].balances[participants[i]]);
            pendingBalances[participants[i]] -= rounds[currentRound].balances[participants[i]];
        }

        successfulRounds[currentRound] = true;

        sendAckToEthereum(currentRound); // Important to unlock ethereum contract only when all balances have been distributed
    }

    /**
     * Once all balances have been distributed, sends an ACK to unlock the contract in ethereum, and repeat the prcoess
     */
    function sendAckToEthereum(uint256 roundIdReceived) external {
    ...
    function _ccipReceive(Client.Any2EVMMessage memory any2EvmMessage) internal override {
        ...
        for (uint256 i = 0; i < incommingRound.participants.length; i++) {
            pendingBalances[incommingRound.participants[i]] += incommingRound.balances[participants[i]];
            currentRoundTotalBalance += incommingRound.balances[participants[i]];
        }
    }
}
